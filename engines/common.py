...
class Inference(object):
            ...
            with torch.no_grad():
                for idx, image_dict in enumerate(tbar):
                    ...
                    if self.is_with_dataloader:
                        ...
                        image_data = raw_image[raw_image > 0]
                        characteristics = {
                            ...
                            'range': np.max(image_data) - np.min(image_data),
                            'cv': np.std(image_data) / np.mean(image_data) if np.mean(image_data) > 0 else 0,
                            'percentile_95': np.percentile(image_data, 95),
                            'percentile_99': np.percentile(image_data, 99)
                        }
                        modality = self.predict_modality(characteristics, image_path)
                    ...
                    if self.train_type == "coarse-fine":
                        model_coarse_path = self.config.COARSE_MODEL_PATH
                        model_fine_path = self.config.FINE_MODEL_PATH
                        model_coarse_name = self.config.COARSE_CHECK_POINT_NAME
                        model_fine_name = self.config.CHECK_POINT_NAME
                        if modality == 'PET':
                            model_coarse_path = model_coarse_path.replace('MR_coarse', 'PET_coarse')
                            model_fine_path = model_fine_path.replace('MR_fine', 'PET_fine')
                            
                            model_coarse_name = model_coarse_name.replace('mr', 'pet')
                            model_fine_name = model_fine_name.replace('mr', 'pet')
                    ...
                    # print('2222')
                    # if modality == 'PET':
                    #     model_path = model_path.replace('Demo_CT_fakeMR_250729_234819', 'Demo_CT_fakePET_250730_004834')
                    if self.is_TTA:
    ...
    def predict_modality(self, characteristics, filename):
        """
        基于图像特征和文件名预测模态类型

        Args:
            characteristics: 图像特征字典
            filename: 文件名
        ...
        # 基于文件名的关键词判断
        mr_keywords = ['t1', 't2', 'flair', 'dwi', 'adc', 'mr', 'mri', 'mprage', 'space', 'amos', 'C+Delay', 'InPhase', 'OutPhase', 'T2WI', 'C+Delay', 'C+V', 'C+A', 'C-pre']
        pet_keywords = ['pet', 'fdg', 'suv', 'tracer', 'uptake', 'PET', 'PET-CT', 'psma']

        # 检查文件名中的关键词
        filename_mr_score = sum(1 for keyword in mr_keywords if keyword in filename_lower)
        filename_pet_score = sum(1 for keyword in pet_keywords if keyword in filename_lower)
        ...
        # PET特征判断
        if 0.1 <= max_val <= 50:  # SUV值范围
            feature_pet_score += 2
        if 0.5 <= mean_val <= 10:  # 平均SUV值
            feature_pet_score += 1
        if cv < 1.5:  # PET图像通常变异系数较小
            feature_pet_score += 1

        # MR特征判断
        if max_val > 100:  # MR图像通常有较高的最大值
            feature_mr_score += 1
        if max_val > 1000:
            feature_mr_score += 2
        if cv > 1.0:  # MR图像通常有较大的变异系数
            feature_mr_score += 1

        # 综合判断
        total_pet_score = filename_pet_score * 3 + feature_pet_score
        total_mr_score = filename_mr_score * 3 + feature_mr_score

        if total_pet_score > total_mr_score:
            return "PET"
        elif total_mr_score > total_pet_score:
            return "MR"
        else:
            # 如果得分相同，使用更保守的特征判断
            if max_val <= 50 and mean_val <= 10:
                return "PET"
            else:
                return "MR"
